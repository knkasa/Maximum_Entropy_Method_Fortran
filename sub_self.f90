subroutine  sub_self( rho, numv, numw, wmax, temp,  z, gap )
implicit none
...
integer :: i,j,l,m, iter, numw, numv

double precision, dimension(1:numv) :: v, Bz, Bph, rho
...
do l = 0,ntheta
   ...
   phi = cos(2.0d0*theta(l))
    ...
    cz(i) = cz(i) + ii*pi*T*(lamzm-lamzp)*iz(j)*dtheta / (exc*0.25d0*pi) & 
               * fac(l)
...
do l = 0,ntheta
     ...
     z(i) = z(i) + ii*pi*dtheta*(bs+fp)*Bz(j)*dv*zp(i+j)/(0.25d0*pi*exc2) &
             * fac(l)
    ...
    z(i)  = z(i) + ii*pi*dtheta*(bs+fm)*Bz(j)*dv*zp(i-j)/(0.25d0*pi*exc2)  & 
              * fac(l)
